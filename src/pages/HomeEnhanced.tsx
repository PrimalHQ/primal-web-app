...
/**
 * Enhanced Home component with seen notes filtering
 * This replaces the original Home.tsx with minimal changes
 */
  ...
  return (
    <div class={styles.homeContent}>
      ...
      <div class={styles.normalCentralHeader}>
        ...
        {/* Optional: Show seen notes filtering status */}
        <Show when={isEnabled() && hiddenNotesCount() > 0}>
          <div class={styles.filterStatus}>
            <span>🔍 {hiddenNotesCount()} seen notes filtered</span>
            <Show when={getStats()}>
              <span class={styles.filterStats}>
                • Filter: {(getStats()?.newFilterCapacity * 100).toFixed(1)}% full
              </span>
            </Show>
          </div>
        </Show>
      ...
      <Switch>
        ...
        >
          <div class={styles.noContent}>
            <div class={styles.emptyFiltered}>
              <h3>🎉 All caught up!</h3>
              <p>You've seen all the recent notes in your feed.</p>
              <Show when={isEnabled()}>
